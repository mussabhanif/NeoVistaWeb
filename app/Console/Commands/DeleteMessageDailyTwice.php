...
class DeleteMessageDailyTwice extends Command
{
     ...
     * @var string
     */
    protected $description = 'Delete Firestore documents older than 12 hours';
    ...
    public function handle()
    {
        ...
        $cutoffTime = $currentTime->subHours(12);

        // Step 3: Delete documents older than 12 hours
        foreach ($documents as $document) {
            $createdAt = $document['fields']['createdAt']['timestampValue'] ?? null;

            if ($createdAt && Carbon::parse($createdAt)->lessThan($cutoffTime)) {
                $this->deleteFirestoreDocument($accessToken, $document['name']);
                $this->info("Deleted document: {$document['name']} (created at: $createdAt)");
            }
        }
